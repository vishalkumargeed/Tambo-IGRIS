...
export const components: TamboComponent[] = [
  {
    ...
    description:
      "Customize the dashboard look. Use when the user asks to change dashboard appearance: theme (light/dark), show/hide sections (cards, chart, table), stats display (cards vs radial/pie chart), contributors display (table vs bar chart), card layout (grid/compact/minimal), sidebar width (narrow/default/wide), accent color, or card style (default/bordered/flat). For stats display: pass statsDisplayVariant: 'radial' when user wants a radial/pie chart instead of 4 cards. For contributors: pass showDataTable: true and contributorsDisplayVariant: 'bar' when the user asks for the detailed view of contributors or contributors in bar chart (e.g. 'show me the detailed view of the contributors', 'show me the contributors in bar chart'). That renders both the Contributor Insights card and the bar chart. Pass contributorsDisplayVariant: 'table' when the user wants only the tabular view (e.g. 'show me the contributors in the form of table', 'contributors as table') — this shows only the contributors table, no insights card. Pass reset: true to restore defaults.",
